...
commands: str = {f"{prefix}open account", f"{prefix}balance", f"{prefix}deposit (amt)", f"{prefix}withdraw (amt)"}
...
async def open_account(message: discord.Message, check: bool):
    ...
    if str(author.id) in users:     #if there is already an account
        if check:                   #checks if called directly or through other method
            return False
        else:
        ...
        users[str(author.id)]["organization"] = "none"
        users[str(author.id)]["speed"] = 1
        users[str(author.id)]["health"] = 100
        users[str(author.id)]["inteligence"] = 100
        users[str(author.id)]["strength"] = 50
        users[str(author.id)]["items"] = [{"name": "pencil", "count": 1}]
        await message.channel.send(f'Your bank account has been made {author.mention}. It will be a pleasure doing business with you')
